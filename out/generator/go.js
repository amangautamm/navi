...
exports.getGoMain = getGoMain;
exports.getGoMod = getGoMod;
function getGoMain(config) {
...
function getGoMod(config) {
    const modPath = `github.com/amanblaze/${config.projectName.toLowerCase().replace(/\\s+/g, '-')}`;
    let deps = '';
    if (config.backendFramework === 'Gin')
        deps = 'require github.com/gin-gonic/gin v1.9.1';
    if (config.backendFramework === 'Echo')
        deps = 'require github.com/labstack/echo/v4 v4.11.3';
    if (config.backendFramework === 'Fiber')
        deps = 'require github.com/gofiber/fiber/v2 v2.51.0';
