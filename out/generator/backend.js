...
exports.getBackendDeps = getBackendDeps;
exports.getExpressServerTemplate = getExpressServerTemplate;
exports.getAuthRoutesTemplate = getAuthRoutesTemplate;
exports.getAuthControllerTemplate = getAuthControllerTemplate;
exports.getAuthMiddlewareTemplate = getAuthMiddlewareTemplate;
function getBackendDeps(config) {
    const deps = [];
    ...
    // Feature deps
    if (features.includes('JWT Authentication'))
        deps.push('jsonwebtoken', 'bcryptjs');
    if (features.includes('OAuth (Google/GitHub)'))
        deps.push('passport', 'passport-google-oauth20', 'passport-github2');
    if (features.includes('Payment Gateway (Razorpay)'))
        deps.push('razorpay');
    if (features.includes('Payment Gateway (Stripe)'))
        deps.push('stripe');
    if (features.includes('File Upload (Multer)'))
        deps.push('multer');
    if (features.includes('Email Service (Nodemailer)'))
        deps.push('nodemailer');
    if (features.includes('SMS Service (Twilio)'))
        deps.push('twilio');
    if (features.includes('Socket.io (Real-time)'))
        deps.push('socket.io');
    if (features.includes('Rate Limiting'))
        deps.push('express-rate-limit');
    if (features.includes('API Documentation (Swagger)'))
        deps.push('swagger-ui-express', 'swagger-jsdoc');
    if (features.includes('Logging (Winston)'))
        deps.push('winston', 'morgan');
    if (features.includes('Error Tracking (Sentry)'))
        deps.push('@sentry/node');
    if (features.includes('Caching (Redis)'))
        deps.push('ioredis');
    if (features.includes('Testing (Jest)'))
        deps.push('jest', 'supertest');
    if (features.includes('Role Based Access Control (RBAC)'))
        deps.push('casbin');
    if (features.includes('Cloud Storage (AWS S3)'))
        deps.push('@aws-sdk/client-s3');
    if (features.includes('Background Jobs (BullMQ)'))
        deps.push('bullmq', 'ioredis');
    if (features.includes('GraphQL API (Apollo Server)'))
        deps.push('graphql', '@apollo/server');
    return [...new Set(deps)];
}
function getExpressServerTemplate(config) {
...
function getAuthRoutesTemplate(config) {
