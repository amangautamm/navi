...
export function getDatabaseDeps(config: ProjectConfig): string[] {
  const { database, orm } = config;
  let deps: string[] = [];
  if (orm === 'Mongoose') deps = ['mongoose'];
  if (orm === 'Prisma') deps = ['prisma', '@prisma/client'];
  if (orm === 'Sequelize') deps = ['sequelize'];
  if (orm === 'TypeORM') deps = ['typeorm', 'reflect-metadata'];
  if (database === 'Firebase Firestore') deps.push('firebase-admin');
  if (database === 'Supabase') deps.push('@supabase/supabase-js');
  if (database === 'Redis') deps.push('ioredis');
  
  if (database === 'PostgreSQL' && orm !== 'Prisma') deps.push('pg');
  if (database === 'MySQL' && orm !== 'Prisma') deps.push('mysql2');
  if (database === 'SQLite' && orm !== 'Prisma') deps.push('sqlite3');
  return deps;
}
